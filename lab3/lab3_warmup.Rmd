...
```{r}
plant_info <- c(heights,plant_mass)

```

```{r}
plant_info_matrix <- matrix(plant_info,nrow = 5,byrow = F)
plant_info_matrix
```
```{r}
colnames(plant_info_matrix) <- measured
rownames(plant_info_matrix) <- samples
```
...
```
```{r}
plant_info_means <- colMeans(plant_info_matrix, na.rm = T)
plant_info_means
```
```{r}
plant_info_final <- rbind(plant_info_matrix,plant_info_means)
```
