...
```{r}
map2 <- get_stadiamap(bbox1, maptype = "stamen_terrain", zoom=5)
```
```{r}
ggmap(map2)
```
...
```{r}
ggmap(map2)+
  geom_point(data = lower_48, aes(long, lat), size=1)+
  labs(title = "Recorded Observations of Wolves",
       x= "Longitude",
       y="Latitude")
```
...
```{r}
ggmap(map2)+
  geom_point(data = lower_48, aes(long, lat, fill=pop, color=pop), size=1)+
  labs(title = "Recorded Observations of Wolves",
       x= "Longitude",
       y="Latitude")
```
