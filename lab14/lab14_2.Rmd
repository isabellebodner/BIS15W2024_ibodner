...
ui <- fluidPage(titlePanel("Log 10 Homerange by Txon"),
                selectInput("x", "Select Taxon", choices = c("birds","lake fishes","lizards","mammals","marine fishes","river fishes","tortoises","turtles"), selected = "birds"),
              plotOutput("plot")
)
  ...
  dashboardBody(
    fluidRow(
      box(width = 3,
          ...
          selectInput("x", "Select Population", choices = unique(wolves$pop)), hr()#instead of typing out every population
...
server <- function(input, output, session) { 
  ...
  output$plot <- renderPlot({
    wolves %>% 
  filter(sex!="NA") %>% #filter is the reactive part
  ggplot(aes(x=sex, fill=sex))+
  geom_bar()
  })
