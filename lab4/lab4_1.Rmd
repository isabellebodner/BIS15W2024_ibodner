...
```
#select and make new data frame
```{r}
...
#has letter a but also ends with er
```{r}
...
glimpse(fish)
```
#pull out all of numerics
```{r}
select_if(fish, is.numeric)
```

To select all columns that are *not* a class of data, you need to add a `~`.
#pull out everything that is NOT numeric. the ! acts as a negative
#~ look in all data
...
```{r}
mean(table(mammals$`max. life`))
```
#weird mass, something is off, how is there -999 mass
```{r}
summary(mammals)
```
