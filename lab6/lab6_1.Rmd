...
```
#attaching janitor and clean names with piping- efficient AF
```{r}
...
```{r}
fish %>% 
  mutate(length_mm = length*10) %>% #using existing data to make new data column
...
mammals
```
#makes all values lowercase to make it easier to manipulate, changing from biologically correct format
```{r}
mammals %>%
  mutate_all(tolower)
```
#or keep genus to be more biologcally correct
