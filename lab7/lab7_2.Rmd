...
#group by only works with categorical variables
```{r}
glimpse(penguins)
```
...
penguins %>% 
  group_by(island) %>% 
  summarise(number_NA=sum(is.na(body_mass_g))) #how many NAs in body mass in the islands
```
...
penguins %>% 
  group_by(island) %>% 
  filter(!is.na(body_mass_g)) %>% #pull out all observations with a number value
...
```{r}
penguins %>% 
  group_by(species, island) %>% #are there multiple species on each island
...
```{r}
penguins %>%
  summarize(across(contains("mm"), \(x) mean(x, na.rm = TRUE))) #\(x) to avoid error
```
