...
```
## here() starts at /Users/ilbodner/Desktop/BIS15W2024_ibodner
```
...
```
Not tidy because there multiple observations with the same value, the data is in long format.
...
```r
separate_sb_long <- sydneybeaches_long %>% 
...
separate_sb_long %>% 
  group_by(site) %>% 
  filter(year == "2013") %>% 
  summarise(max_cocci=max(enterococci_cfu_100ml)) %>% 
  arrange(desc(max_cocci))
```
